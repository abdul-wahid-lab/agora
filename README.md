...
Each device's local API is bound to `127.0.0.1` — it's how that device's own UI talks to its own backend, never reachable from other peers. All actual peer-to-peer traffic (discovery, messages, files, calls) goes out over separate LAN-facing sockets, entirely independent of that loopback API.
...
mDNS is the primary path; UDP broadcast is the fallback for networks that filter mDNS traffic. Both are checked — a peer only needs to be reachable by one of them to show up.
...
Executable/installable files get an extra security interstitial (two required checkboxes + a countdown) before the accept path is even reachable — see [Security posture](#security-posture-honest-as-of-now).
...
Signaling (offer/answer/ICE) piggybacks on the same direct WebSocket messaging already uses — no separate signaling server, cloud or otherwise. Once connected, audio/video flows directly between the two devices.
...
In dev mode, Electron spawns the backend via the dev virtualenv's `python.exe` directly for fast iteration. In a packaged build, it spawns the frozen `agora-backend.exe` instead — the app has no dependency on Python being installed on the machine it's run on.
